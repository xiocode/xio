...
#    p = re.compile(r'\"ticket\":\(\"(.*?)\"\)')
    ...
    try:
        ...
        login_url = 'http://kandian.com/logon/do_crossdomain.php?action=login&callback=sinaSSOController.doCrossDomainCallBack&scriptId=ssoscript0&client=ssologin.js(v1.4.2)&_=1347873165314'
        resp = session.get(login_url)
        print resp.text
        login_url = 'http://login.t.cn/sinaurl/sso.json?action=login&uid=2684726573&callback=sinaSSOController.doCrossDomainCallBack&scriptId=ssoscript1&client=ssologin.js(v1.4.2)&_=1347873165316'
        resp = session.get(login_url)
        print resp.text
        login_url = 'http://weibo.com/sso/login.php?callback=sinaSSOController.customLoginCallBack&ticket='+login_ticket+'&ssosavestate=1350465165&client=ssologin.js(v1.4.2)&_=1347873165393'
        resp = session.get(login_url)
        print resp.text
